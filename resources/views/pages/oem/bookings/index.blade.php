...
<div>
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('oem.bookings.create') }}" class="ml-4 inline-flex items-center rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-500">+ New Booking</a>
        @endif
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($jobs as $job)
                <tr class="hover:bg-gray-50 cursor-pointer" onclick="window.location='{{ route('oem.bookings.show', $job) }}'">
                    <td class="px-6 py-4 text-sm font-medium text-blue-600">{{ $job->job_number ?? '—' }}</td>
                    <td class="px-6 py-4 text-sm text-gray-500">@if($job->isTransport()){{ $job->pickupLocation?->company_name }} &rarr; {{ $job->deliveryLocation?->company_name }}@else Yard @endif</td>
