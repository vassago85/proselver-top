...
/*
 * The trip planner page. Two columns:
 *
 *  - Left: the sequenced list of stops. Each stop can be reordered
 *    (move up / down), removed, or marked arrived/departed (the same
 *    actions the driver uses on My-Day, exposed here so dispatchers
 *    can override).
 *  - Right: side panel of attachable jobs for the trip's date and a
 *    cluster of buttons that insert non-job stops (positioning, COF,
 *    weighbridge, fuel, custom waypoint).
 *
 * The drag-and-drop reorder mentioned in the plan is implemented as
 * up/down arrow buttons here — same outcome (reorderStops on the
 * planner service) without needing a JS sortable lib. Easy to swap to
 * a real sortable later because everything funnels through TripPlanner.
 */
...
class extends Component
{
    ...
    public function mount(Trip $trip): void
    {
        ...
        abort_unless((int) $trip->company_id === (int) $this->company->id, 404);
    ...
    {
        $this->trip->refresh()->load(['driver', 'stops.job.brand', 'stops.job.pickupLocation', 'stops.job.deliveryLocation', 'stops.location', 'startLocation', 'endLocation']);
    }
    ...
    public function attachJob(int $jobId, TripPlanner $planner): void
    {
        ...
        $job = Job::query()->where('id', $jobId)->where('company_id', $this->company->id)->first();
        if (!$job) { return; }

        try {
            $planner->attachJob($this->trip, $job);
        } catch (\Throwable $e) {
    ...
    public function detachJob(int $jobId, TripPlanner $planner): void
    {
        ...
        try {
            $planner->detachJob($this->trip, $job);
        } catch (\Throwable $e) {
            $this->errorMessage = $e->getMessage();
            return;
        }
    ...
    // ---------------------------------------------------------------------
    // Stop ops (reorder, remove, arrive/depart)
    // ---------------------------------------------------------------------
    ...
    public function moveStop(int $stopId, string $direction, TripPlanner $planner): void
    {
        ...
        try {
            $planner->reorderStops($this->trip, $map);
        } catch (\Throwable $e) {
            $this->errorMessage = $e->getMessage();
            return;
        }
    ...
    {
        // Side panel: jobs available to attach. Scope to the same
        // company as the trip, no trip already assigned, scheduled
        // within ±3 days of the trip date so we don't drown the panel
        // in unrelated future work.
        ...
        return [
            ...
            'waypointTypes'   => [
                TripStop::TYPE_POSITIONING          => ['label' => 'Positioning leg', 'icon' => '↻', 'color' => 'indigo'],
                TripStop::TYPE_WAYPOINT_COF         => ['label' => 'COF check',       'icon' => '✓', 'color' => 'emerald'],
                TripStop::TYPE_WAYPOINT_WEIGHBRIDGE => ['label' => 'Weighbridge',     'icon' => '⚖', 'color' => 'sky'],
                TripStop::TYPE_WAYPOINT_FUEL        => ['label' => 'Fuel stop',       'icon' => '⛽', 'color' => 'amber'],
                TripStop::TYPE_WAYPOINT_OTHER       => ['label' => 'Waypoint',        'icon' => '◯', 'color' => 'slate'],
            ],
        ];
    }
};
...
?>
<div class="space-y-6">
    <x-slot:header>Trip planner</x-slot:header>
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex flex-wrap items-start justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('customer.trips.index') }}" class="text-sm font-medium text-slate-500 hover:text-slate-800">← All trips</a>
    ...
    <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
            ...
            <div class="rounded-xl border border-slate-200 bg-white shadow-sm">
                ...
                <ol class="divide-y divide-slate-100">
                    @forelse($trip->stops as $stop)
                        @php
                            $typeMeta = $waypointTypes[$stop->stop_type] ?? null;
                            ...
                            <div class="flex flex-wrap items-start gap-3">
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="flex flex-wrap items-center gap-2">
                                        ...
                                        @if($isJob && $stop->job)
                                            <a href="{{ route('customer.orders.show', $stop->job) }}"
                                    ...
                                    <div class="mt-1 text-xs text-slate-500">
                                        {{ $locationName }}
                                        @if($stop->expected_at)
                                            · expected {{ $stop->expected_at->format('H:i') }}
                                        @endif
                                    </div>
                                    @if($stop->notes)
                                        <p class="mt-1 text-xs italic text-slate-500">{{ $stop->notes }}</p>
                                    @endif
                                    <div class="mt-2 flex flex-wrap items-center gap-3 text-[11px] text-slate-500">
                                        @if($stop->arrived_at)
                                            <span>arrived <span class="font-medium text-slate-700">{{ $stop->arrived_at->format('H:i') }}</span></span>
                                        @endif
                                        @if($stop->departed_at)
                                            <span>departed <span class="font-medium text-slate-700">{{ $stop->departed_at->format('H:i') }}</span></span>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-none flex-wrap items-center gap-1">
                                    @if(!$stop->arrived_at && !in_array($trip->status, [Trip::STATUS_COMPLETED, Trip::STATUS_CANCELLED], true))
                                        <button wire:click="markArrived({{ $stop->id }})"
                                            class="rounded-md bg-emerald-50 px-2 py-1 text-[11px] font-semibold text-emerald-700 ring-1 ring-emerald-200 hover:bg-emerald-100">
                                            Arrived
                                        </button>
                                    @endif
                                    @if($stop->arrived_at && !$stop->departed_at && !in_array($trip->status, [Trip::STATUS_COMPLETED, Trip::STATUS_CANCELLED], true))
                                        <button wire:click="markDeparted({{ $stop->id }})"
                                            class="rounded-md bg-blue-50 px-2 py-1 text-[11px] font-semibold text-blue-700 ring-1 ring-blue-200 hover:bg-blue-100">
                                            Departed
                                        </button>
                                    @endif
                                    @if($canEdit)
                                        <button wire:click="moveStop({{ $stop->id }}, 'up')" title="Move up"
                                            class="rounded-md border border-slate-200 bg-white px-1.5 py-1 text-xs text-slate-600 hover:bg-slate-50">↑</button>
                                        <button wire:click="moveStop({{ $stop->id }}, 'down')" title="Move down"
                                            class="rounded-md border border-slate-200 bg-white px-1.5 py-1 text-xs text-slate-600 hover:bg-slate-50">↓</button>
                                        <button wire:click="openWaypoint({{ $stop->sequence }}, 'waypoint_other')" title="Insert below"
                                            class="rounded-md border border-slate-200 bg-white px-1.5 py-1 text-xs text-slate-600 hover:bg-slate-50">+</button>
                                        <button wire:click="removeStop({{ $stop->id }})" wire:confirm="Remove this stop?"
                                            class="rounded-md border border-rose-200 bg-white px-1.5 py-1 text-xs text-rose-600 hover:bg-rose-50">✕</button>
                                    @endif
                                </div>
                            </div>

                            @if($canEdit && $insertAfterSequence === $stop->sequence && $waypointType)
                                @include('pages.customer.trips._waypoint-form')
                            @endif
                        </li>
                    @empty
                        <li class="px-5 py-12 text-center text-sm text-slate-500">
                            No stops yet. Drag a job from the right panel or add a waypoint to begin.
                        </li>
                    @endforelse
        ...
        {{-- RIGHT: attachable jobs --}}
        <div>
            <div class="rounded-xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-100 px-5 py-3">
                    <h2 class="text-sm font-semibold text-slate-900">Unassigned jobs</h2>
                    <p class="mt-0.5 text-xs text-slate-500">Within ±3 days of {{ $trip->trip_date->format('j M') }}. Click to add to the trip.</p>
                </div>
                <ul class="max-h-[640px] divide-y divide-slate-100 overflow-y-auto">
                            ...
                            <div class="mt-1 flex items-center justify-between text-[11px] text-slate-500">
                                ...
                                @if($canEdit)
                                    <button wire:click="attachJob({{ $job->id }})"
                                        class="rounded-md bg-blue-600 px-2 py-1 text-[11px] font-semibold text-white hover:bg-blue-500">
                                        + Add
                                    </button>
                                @endif
                            </div>
                    ...
                    @empty
                        <li class="px-5 py-8 text-center text-xs text-slate-500">
                            No unassigned jobs in this window.
                        </li>
                    @endforelse
                </ul>
            </div>

            @if($trip->jobs->isNotEmpty())
                <div class="mt-4 rounded-xl border border-slate-200 bg-white shadow-sm">
                    <div class="border-b border-slate-100 px-5 py-3">
                        <h2 class="text-sm font-semibold text-slate-900">Vehicles on this trip</h2>
                    </div>
                    <ul class="divide-y divide-slate-100">
                        @foreach($trip->jobs as $j)
                            <li class="flex items-center justify-between px-5 py-2 text-xs">
                                <span class="font-medium text-slate-700">{{ $j->vin ?? ('JOB-' . $j->id) }}</span>
                                @if($canEdit)
                                    <button wire:click="detachJob({{ $j->id }})" wire:confirm="Remove this vehicle from the trip?"
                                        class="text-rose-600 hover:text-rose-800">Detach</button>
                                @endif
                            </li>
                        @endforeach
                    </ul>
