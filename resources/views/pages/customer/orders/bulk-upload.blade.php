...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    /**
     * Confirm-address panel state.  Populated after buildPreview() by
     * scanning previewRows for pickup/delivery cells that would auto-
     * create a new location on commit and calling GeocodingService for
     * each.  Keyed by JobBulkImporter::normaliseAddressKey(rawName).
     *
     * Shape per entry:
     *   [
     *     'key' => 'anchorautobodybuildersscc',
     *     'raw' => 'ANCHOR AUTO BODY BUILDERS CC',
     *     'sides' => ['pickup'],
     *     'row_count' => 3,
     *     'suggestions' => [ ['formatted_address' => '...', 'city' => ...], ... ],
     *     'selection' => [
     *       'choice' => 'kept' | 'suggestion' | 'custom',
     *       'index'  => 0,                          // for choice = suggestion
     *       'address' => '...', 'city' => '...',    // for choice = custom
     *     ],
     *   ]
     */
    public array $addressConfirmations = [];

    /** Toggle for the confirmation panel details -- collapsed by default when everything is 'kept'. */
    ...
    public function buildPreview(JobBulkImporter $importer): void
    {
        ...
        // Suggest Google-confirmed addresses for every unique pickup /
        // delivery string that would otherwise auto-create as a stub.
        // Runs once per unique raw string (dedup on locationKey) so a
        // 500-row file with 20 unique dealers only makes 20 API calls.
    ...
    /**
     * Pick a Google suggestion for one unmatched raw address.  The
     * choice is remembered on $addressConfirmations and applied at
     * commit time -- resolveLocation() uses the confirmed address
     * instead of stubbing raw-name-as-address.
     */
    ...
    /**
     * Keep the raw pickup / delivery text as the address -- explicit
     * opt-out so the operator's decision is recorded (rather than an
     * unconfirmed row silently using the stub).
     */
    ...
    /**
     * Save a custom address (typed via Places autocomplete inside the
     * confirmation panel).  Called from Alpine when the user selects a
     * place -- lat/lng are passed through so we don't re-geocode on
     * commit.
     */
    ...
    {
        // Turn the panel's selection state into the structured shape
        // JobBulkImporter::resolveLocation() consumes -- one entry per
        // raw address the operator actually confirmed (either picked a
        // Google suggestion or typed a custom Places result).  "kept"
        // selections skip through: their location still lands as a
        // raw-name stub so ops can fix it later from the cleanup UI.
        $confirmedForCommit = [];
                        ...
                        <div class="flex flex-wrap items-baseline justify-between gap-2">
                            <div>
                                ...
                                <p class="mt-1 text-xs text-slate-600">
                                    These pickup / delivery names are about to be added to your address book.
