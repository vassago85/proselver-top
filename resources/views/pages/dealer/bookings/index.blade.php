...
?>
<div>
    <x-slot:header>My Bookings</x-slot:header>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 cursor-pointer" onclick="window.location='{{ route('dealer.bookings.show', $job) }}'">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-500">@if($job->isTransport()){{ $job->fromHub?->name }} → {{ $job->toHub?->name }}@else Yard @endif</td>
                ...
                @empty
                <tr><td colspan="4" class="px-6 py-12 text-center text-sm text-gray-500">No bookings yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
