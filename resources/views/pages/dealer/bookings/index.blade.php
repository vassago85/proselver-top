...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        $user = auth()->user();
        if (!$user->hasAnyPermission(['view_all_bookings', 'view_own_bookings'])) {
            abort(403);
        }
    ...
    public function with(): array
    {
        ...
        if (!$company) return ['jobs' => collect(), 'canViewAll' => false, 'canUploadDocs' => false, 'canCreateBooking' => false];

        $canViewAll = $user->hasPermission('view_all_bookings');
        ...
        if (!$canViewAll || $this->viewMode === 'mine') {
        ...
        return [
            ...
            'canCreateBooking' => $user->hasPermission('submit_booking'),
        ];
    }
...
<div>
    ...
    <div class="flex justify-between items-center mb-6">
        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search by job #, VIN, make/model, route, driver, or PO..." class="w-full max-w-md rounded-lg border border-gray-300 px-4 py-2.5 text-sm">
        @if($canCreateBooking)
        <a href="{{ route('dealer.bookings.create') }}" class="ml-4 inline-flex items-center rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-500 whitespace-nowrap">+ New Booking</a>
        @endif
    </div>
    @if($canViewAll)
        ...
        <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <th wire:click="sort('job_number')" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer hover:text-gray-700 select-none">
                        <span class="inline-flex items-center gap-1">Job #
                            @if($sortBy === 'job_number')
                                <svg class="h-3 w-3 {{ $sortDir === 'asc' ? '' : 'rotate-180' }}" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" fill="none"><path d="m18 15-6-6-6 6"/></svg>
                            @endif
                        </span>
                    </th>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3">
                        <a href="{{ route('dealer.bookings.show', $job) }}" class="text-sm font-medium text-blue-600 hover:text-blue-800">{{ $job->job_number ?? '—' }}</a>
                    </td>
                    ...
                    <td class="px-4 py-3 text-center" onclick="event.stopPropagation()">
                        <div class="flex items-center justify-center gap-1">
                            <a href="{{ route('dealer.bookings.show', $job) }}" class="rounded-md bg-gray-50 px-2 py-1 text-xs font-medium text-gray-600 hover:bg-gray-100" title="View Details">
                                <svg class="h-4 w-4" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" fill="none"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>
                            </a>
                            @if($canUploadDocs)
                            <button wire:click="openPoUpload({{ $job->id }})" class="rounded-md bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700 hover:bg-blue-100" title="Upload PO">
                                <svg class="h-4 w-4" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" fill="none"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" x2="12" y1="3" y2="15"/></svg>
                            </button>
                            @endif
                        </div>
                    </td>
                ...
                @empty
                <tr><td colspan="9" class="px-6 py-12 text-center text-sm text-gray-500">No bookings yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
        ...
        <div class="w-full max-w-md mx-4 bg-white rounded-2xl shadow-2xl overflow-hidden">
            <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                ...
                <button wire:click="closePoUpload" class="rounded-full p-1 text-gray-400 hover:text-gray-600 hover:bg-gray-100">
                    <svg class="h-5 w-5" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" fill="none"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
                </button>
            </div>
