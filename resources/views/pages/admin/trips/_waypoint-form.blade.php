{{--
    Ops-side waypoint insertion form. Mirrors the dealer planner's
    partial — kept under admin/trips/ rather than shared so each side
    can evolve its own copy (e.g. ops might want to add a "scheduled-by"
    field that dealers don't need).
--}}
<div class="mt-3 rounded-md border border-slate-200 bg-slate-50 p-3">
    ...
    <div class="mt-3 flex items-center gap-2">
        <button wire:click="saveWaypoint" class="rounded-md bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-500">Add stop</button>
        <button wire:click="cancelWaypoint" type="button" class="text-xs font-medium text-slate-500 hover:text-slate-800">Cancel</button>
    </div>
</div>
